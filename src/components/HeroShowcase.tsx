...
export function HeroShowcase() {
  ...
  const advanceIfReady = useCallback((nextIdx: number) => {
    if (loadedSet.current.has(IMAGES[nextIdx].src)) {
  ...
  const markLoaded = useCallback((src: string) => {
    ...
    if (src === IMAGES[pendingIndex.current].src) {
      setDisplayIndex(pendingIndex.current);
    }
  }, []);

  // Preload all images via native browser fetch
  useEffect(() => {
  ...
  // Timer: queue the next index, advance only if loaded
  useEffect(() => {
